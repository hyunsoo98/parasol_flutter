...
### 2. S3 버킷 생성
```bash
aws s3 mb s3://eye-tracking-videos --region ap-northeast-2
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:dynamodb:ap-northeast-2:*:table/eye-tracking-results"
    },
    {
      "Effect": "Allow",
      "Action": [
      ...
      "Resource": "arn:aws:sqs:ap-northeast-2:*:eye-tracking-queue"
    }
  ]
}
```
...
aws lambda create-function \
    ...
    --environment Variables='{
        ...
        "SQS_QUEUE_URL":"https://sqs.ap-northeast-2.amazonaws.com/YOUR_ACCOUNT/eye-tracking-queue",
...
aws lambda create-event-source-mapping \
    --function-name eye-tracking-process \
    --event-source-arn arn:aws:sqs:ap-northeast-2:YOUR_ACCOUNT_ID:eye-tracking-queue \
    --batch-size 1 \
    --region ap-northeast-2
```
...
```dart
static const String _baseUrl = 'https://YOUR_API_ID.execute-api.ap-northeast-2.amazonaws.com/prod';
```
...
```bash
# Upload 테스트
curl -X POST https://YOUR_API_ID.execute-api.ap-northeast-2.amazonaws.com/prod/upload \
  -H "Content-Type: application/json" \
  -d '{"video_data":"...","user_id":"test"}'

# Status 테스트  
curl https://YOUR_API_ID.execute-api.ap-northeast-2.amazonaws.com/prod/status/ANALYSIS_ID
```
...
🎉 **배포 완료!** 이제 AWS 순수 환경에서 확장 가능한 비동기 eye tracking 분석 시스템이 준비되었습니다!1