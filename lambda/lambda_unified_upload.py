...
ANALYSIS_CONFIGS = {
    ...
    'finger-tapping': {
        ...
        'sqs_queue_url': os.environ.get('FINGER_TAPPING_QUEUE', 'https://sqs.us-west-1.amazonaws.com/YOUR_ACCOUNT/finger-tapping-queue.fifo'),
        'dynamodb_table': 'ParkinsonAnalysis',
        'content_type': 'video/mp4',
        'file_extension': '.mp4'
    },
    'voice-analysis': {
        's3_prefix': 'audio/voice-analysis/',
        'sqs_queue_url': os.environ.get('VOICE_ANALYSIS_QUEUE', 'https://sqs.us-west-1.amazonaws.com/YOUR_ACCOUNT/voice-analysis-queue.fifo'),
        'dynamodb_table': 'ParkinsonAnalysis',
    ...
    try:
        ...
        if not skip_db:
            try:
                ...
                table.put_item(Item={
                    'analysis_id': analysis_id,  # camelCase -> snake_case로 변경
                    'user_id': user_id,
                    'status': 'uploaded',
                    'timestamp': timestamp,
                    's3_key': s3_key,
        ...
        if config['sqs_queue_url']:  # SQS URL이 있는 경우만 전송
            try:
                ...
                try:
                    table = dynamodb.Table(config['dynamodb_table'])
                    table.update_item(
                        Key={'analysis_id': analysis_id},  # camelCase -> snake_case로 변경
