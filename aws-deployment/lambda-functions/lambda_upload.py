    ...
    try:
        ...
        return {
            ...
            'body': json.dumps({
                ...
                'analysis_id': analysis_id,
                'status': 'processing',
                'message': '분석이 시작되었습니다. 상태를 확인하여 결과를 받아보세요.',
                'file_size': file_size,
        ...
        print(f"Upload Lambda 오류: {str(e)}")
...
def estimate_processing_time(file_size: int) -> int:
    """파일 크기 기반 예상 처리 시간 (초)"""
    # 1MB당 약 10초 예상
    estimated_seconds = (file_size / (1024 * 1024)) * 10
    return max(30, min(900, int(estimated_seconds)))  # 30초~15분 범위