...
class _StructuredEyeTestScreenState extends State<StructuredEyeTestScreen> {
  ...
  // 얼굴 위치 허용 범위 상수 (매우 관대하게 설정)
  static const double kMinRelFace = 0.005; // 최소 얼굴 크기 (0.5%) - 매우 완화
  static const double kMaxRelFace = 1.0;   // 최대 얼굴 크기 (100%) - 완전 관대
  static const double kCenterJumpTol = 0.1; // 중심점 이동 허용 범위 (10%)
  static const double kPosTol = 0.3; // 위치 허용 오차 (30%)
  ...
  Future<void> _processFrame(CameraImage image) async {
    try {
      ...
      if (faces.isNotEmpty) {
        ...
        if (faceRatio > 0.0001) { // 0.01% 이상이면 얼굴로 인정 (극도로 관대)
          print('얼굴 인정 - 비율: ${(faceRatio * 100).toStringAsFixed(2)}%');
          _recentFaceDetected = true;
          _faceDetectedOnce = true; // 한번이라도 인식되면 플래그 설정
  ...
  void _recordFaceDetected() {
    setState(() {
      _faceDetected = true;
      _faceDetectedOnce = true; // 한번 인식되면 플래그 설정
    });
  ...
  void _recordNoFaceDetected() {
    // 한번이라도 인식된 적이 있으면 계속 인식된 것처럼 처리
    if (_faceDetectedOnce) {
      setState(() {
        _faceDetected = true; // 지속적으로 true 유지
      });
      _confidence = 0.5; // 중간 정도 신뢰도 유지
    } else {
      setState(() {
        _faceDetected = false;
      });
  ...
  void _addAnalysisFrame() {
    ...
    if (!_faceDetected) {
      ...
      if (_forceContinueMode && _confidence > 0.3) { // 강제 진행 모드 기준 완화 (0.5→0.3)
    ...
    final analysisData = {
      ...
      'stable': _lastStable,
      'eye_position_quality': _confidence > 0.6 ? 'high' : (_confidence > 0.3 ? 'medium' : 'low'), // 품질 판정 기준 완화
  ...
  Future<void> _speak(String text) async {
    try {
      ...
      await Future.delayed(const Duration(milliseconds: 1000)); // TTS 완료 대기
    } catch (e) {
      print('TTS 오류: $e');
    }
  }
  ...
  void _proceedToFaceAlignment() async {
    ...
    _phaseTimer = Timer.periodic(const Duration(milliseconds: 500), (timer) {
      if (_faceDetected && _confidence > 0.1) { // 얼굴 정렬 통과 기준 매우 완화 (0.4→0.1)
        timer.cancel();
        _proceedToGazeTest();
      }
    });
  ...
  void dispose() {
    ...
    _flutterTts.stop();
    super.dispose();
    // MediaPipe service cleanup removed as it's not defined
  }
}
